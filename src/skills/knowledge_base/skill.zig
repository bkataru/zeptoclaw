...
pub const skill = struct {
    ...
    fn loadIndex(allocator: Allocator) !void {
        ...
        const file = std.fs.cwd().openFile(file_path, .{}) catch |err| {
            if (err == error.FileNotFound) {
                ...
                index = Index{
                    ...
                    .indexed_at = "",
                    .files = try std.ArrayList(Index.File).initCapacity(allocator, 0),
                };
                return;
            }
            return err;
        ...
        var files = try std.ArrayList(Index.File).initCapacity(allocator, 0);
        ...
        for (files_val.array.items) |file_val| {
            ...
            var headers = try std.ArrayList(Index.Header).initCapacity(allocator, 0);
    ...
    fn handleList(ctx: *ExecutionContext) !SkillResult {
        ...
        var count: usize = 0;
        var files = try std.ArrayList([]const u8).initCapacity(ctx.allocator, 0);
        defer {
            for (files.items) |f| {
    ...
    fn handleTree(ctx: *ExecutionContext) !SkillResult {
        ...
        for (index.?.files.items) |file| {
            ...
            if (!entry.found_existing) {
                entry.value_ptr.* = try std.ArrayList([]const u8).initCapacity(ctx.allocator, 0),
            }
