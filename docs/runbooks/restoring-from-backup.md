...
Persona markdown and memory live in private **github.com/bkataru/barvis**, checked out at `~/.zeptoclaw/workspace`. `~/.openclaw/workspace` is a leftover. `barvis-sync.timer` commits the ZeptoClaw workspace every 30 minutes (journals + `MEMORY.md`).
