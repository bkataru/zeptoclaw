...
3. Set `NVIDIA_API_KEY` (and `GATEWAY_AUTH_TOKEN`) in the **user** systemd unit or `~/.config/zeptoclaw/env` (`chmod 600`). Copy the same NVIDIA key to `~/.config/zeptoclaw/nim.env` for memory oneshots.
...
6. Optional: `systemctl --user enable --now barvis-memory-update.timer` (2-hour compact).
7. Confirm `journalctl --user -u zeptoclaw-gateway.service` shows `connection status=connected`.
8. Pair WhatsApp if stderr asks for a QR.
