//! Shell2HTTP Server
//! HTTP server on port 9001 with basic auth
...
const log = std.log.scoped(.shell2http_server);
...
pub const Shell2HTTPServer = struct {
    ...
    pub fn stop(self: *Shell2HTTPServer) void {
        self.running = false;
    }
...
pub fn main() !void {
    ...
    // Initialize server
    var server = try Shell2HTTPServer.init(allocator, config);
    defer server.deinit();
