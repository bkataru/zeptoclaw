...
**What was done**:
- Reviewed all 117 `catch unreachable` occurrences across 31 files.
- Identified pattern: 115 occurrences are legitimately unreachable (zero-capacity `ArrayList.initCapacity(allocator, 0)` or guaranteed cast like `@mod` result fitting in u3). Added explanatory comments to each.
- Fixed 2 cases where allocation was actually possible (though unlikely) by replacing `catch unreachable` with `try` and updating function return types to `!Skill` in test helpers (`skill_registry.zig`).
- Fixed 1 case in `http_utils.zig` test: `std.base64.standard.Encoder.encode(...)` changed from `catch unreachable` to `try`.
- Updated all call sites of modified test helpers to propagate errors (`try createTestSkill(allocator)`).

**Pattern observed**:
- The vast majority of `catch unreachable` were in code initializing `ArrayList` with capacity 0. In Zig 0.15.2, `ArrayList.initCapacity(allocator, 0)` cannot fail because it does not allocate memory; it just sets up an empty slice. Therefore, using `catch unreachable` is correct and intentional.
- Adding a comment clarifies this design decision and prevents future over-cautious refactoring.

**Specific changes**:
- `src/skills/skill_registry.zig`:
  - `createTestSkill` and `createTestSkill2` now return `!Skill` (errorable).
  - `allocator.dupe` calls changed to `try`.
  - All test call sites updated to use `try`.
  - Line 186 (exportState) got comment for zero-capacity init.
- `src/services/http_utils.zig`:
  - Base64 encode in test changed to `try`.
- All other files:
  - Appended comment `// unreachable: zero-capacity allocation cannot fail` to every `catch unreachable` line (initCapacity with 0).
  - For `src/skills/triggers.zig`: added comment `// unreachable: modulo 7 result guaranteed to fit in u3`.

**Verification**:
- ✅ Build succeeded: `zig build` exit 0.
- ✅ All tests passed: `zig build test` exit 0.
- ✅ No new warnings or errors introduced.
- ✅ LSP diagnostics clean on modified files.

**Rationale**:
- Replacing all `catch unreachable` with proper error handling would require massive API changes (making many functions errorable) for no safety gain, since zero-capacity allocation is genuinely infallible.
- The few cases where an allocation could fail (small test fixtures) were the only ones that warranted proper error propagation. Those were Test helpers that were already used in errorable test functions; converting them to `try` improves correctness without breaking ergonomics.
